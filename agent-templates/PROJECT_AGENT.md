...
| `subjects.js` | none | **The cast-and-props list. Declare everything that recurs BEFORE rendering any shot.** Free, no generation. `--declare --name "the brass telegraph key" --kind item --alias "telegraph key"` registers a subject; `--list` shows every subject and whether it is anchored; `--audit` exits non-zero if anything declared lacks an approved reference, so it can gate a run before money is spent. **An empty list fails the audit too** — it is not a clean bill of health, it is the state in which a prop goes un-drawn. `reference_sheet.js` registers automatically, so building a reference is declaring it. `generate_video.js` REFUSES to render a shot whose prompt names a declared subject that is not wired as a `--ref-source-id` (`--allow-unreferenced` overrides). |
