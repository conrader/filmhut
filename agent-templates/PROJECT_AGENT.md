...
| `reference_sheet.js` | `image-compose` | Builds a four-panel multi-angle sheet for a character (front/profile/back/face) or an item (front/three-quarter/reverse/macro detail). **EVERY subject that appears in more than one shot needs a reference — objects as well as faces.** A prop drifts exactly like a face does, and for the same reason: nothing anchors it, so each shot invents it again. A hero prop present in five of six shots and described only in prose came back as a different object in shot 1 than in the rest. If it appears twice, it gets a reference: props, vehicles, signage, animals, a distinctive garment. Flags: `--name`, `--kind character\|item`, `--description`, `--ref-source-id`, `--size`, `--print` to see the prompt for free, and **`--single`** for one composed view instead of four panels — the right choice for a small rigid object the model has no confident idea of, where four panels just means four chances to disagree. Wide sizes (`1568x672`) tile four small-object panels far better than near-square ones. |
...
| `subjects.js` | none | **The cast-and-props list, and the reference checklist. Run `--checklist` BEFORE rendering any shot.** Free, no generation. `--checklist` asks what a film NEEDS rather than what someone happened to declare: cast, locations, props. **A project with no location declared FAILS** — every film happens somewhere, and a set drifts exactly as a face does; one finished film passed its audit clean with no location reference at all. `--declare --name "the brass telegraph key" --kind item --alias "telegraph key"` registers a subject; `--list` shows each subject and whether it is anchored; `--audit` exits non-zero if anything declared lacks an approved reference. `reference_sheet.js` registers automatically, so building a reference is declaring it. `generate_video.js` REFUSES to render a shot whose prompt names a declared subject that is neither wired as a `--ref-source-id` nor in the anchor's lineage (`--allow-unreferenced` overrides). |
